...
struct TempRoot {
    ...
    TempRoot() {
        ...
        path = std::filesystem::temp_directory_path() /
               ("tsro-store-" + std::to_string(++counter) + "-" +
...
TEST(profiles, a_first_run_yields_defaults_rather_than_an_error) {
    ...
    CHECK_EQ(config.appearance.font_size, Config::defaults().appearance.font_size);
...
TEST(profiles, a_saved_profile_round_trips) {
    ...
    config.user_list.max_visible_users = 9;
    config.indicators.commander.icon_color = Color{0, 255, 255, 255};
    ...
    CHECK_EQ(loaded.user_list.max_visible_users, 9);
    CHECK_EQ(loaded.indicators.commander.icon_color.to_hex(), std::string("#00FFFFFF"));
}
...
TEST(profiles, export_and_import_round_trip) {
    ...
    UserOverride ov;
    ov.name_color = Color{255, 0, 128, 255};
    config.user_overrides["kZ9="] = ov;
    ...
    CHECK_EQ(imported.find_user_override("kZ9=")->name_color->to_hex(), std::string("#FF0080FF"));
}
