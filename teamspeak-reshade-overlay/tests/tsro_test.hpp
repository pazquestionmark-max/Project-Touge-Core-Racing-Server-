...
#ifndef TSRO_TEST_HPP
#define TSRO_TEST_HPP
...
#define TSRO_CONCAT_INNER(a, b) a##b
#define TSRO_CONCAT(a, b) TSRO_CONCAT_INNER(a, b)

#define TEST(suite, name)                                                               \
    static void TSRO_CONCAT(tsro_test_body_, __LINE__)();                               \
    static ::tsro_test::Registrar TSRO_CONCAT(tsro_test_reg_, __LINE__)(                \
        #suite, #name, &TSRO_CONCAT(tsro_test_body_, __LINE__));                        \
    static void TSRO_CONCAT(tsro_test_body_, __LINE__)()

#define CHECK(cond)                                                                     \
    do {                                                                                \
        if (!(cond)) ::tsro_test::fail(__FILE__, __LINE__, "expected: " #cond);          \
    } while (false)

#define CHECK_EQ(actual, expected)                                                      \
    do {                                                                                \
        const auto& tsro_a = (actual);                                                  \
        const auto& tsro_b = (expected);                                                \
        if (!(tsro_a == tsro_b)) {                                                       \
            ::tsro_test::fail(__FILE__, __LINE__,                                        \
                              std::string(#actual " == " #expected "\n         actual:   ") + \
                                  ::tsro_test::show(tsro_a) + "\n         expected: " +  \
                                  ::tsro_test::show(tsro_b));                            \
        }                                                                                \
    } while (false)
...
#define CHECK_NE(actual, expected)                                                      \
    do {                                                                                \
        if ((actual) == (expected))                                                      \
            ::tsro_test::fail(__FILE__, __LINE__, #actual " should differ from " #expected); \
    } while (false)

#define CHECK_NEAR(actual, expected, tolerance)                                         \
    do {                                                                                \
        const double tsro_a = static_cast<double>(actual);                              \
        const double tsro_b = static_cast<double>(expected);                            \
        const double tsro_t = static_cast<double>(tolerance);                           \
        if (!((tsro_a - tsro_b) <= tsro_t && (tsro_b - tsro_a) <= tsro_t)) {             \
            ::tsro_test::fail(__FILE__, __LINE__,                                        \
                              std::string(#actual " ~= " #expected "\n         actual:   ") + \
                                  ::tsro_test::show(tsro_a) + "\n         expected: " +  \
                                  ::tsro_test::show(tsro_b));                            \
