...
#ifndef TSRO_PROFILE_STORE_HPP
#define TSRO_PROFILE_STORE_HPP
...
class ProfileStore {
public:
    /// `%APPDATA%\TeamSpeakReShadeOverlay` on Windows; `$XDG_CONFIG_HOME/tsro` (or
    /// `~/.config/tsro`) elsewhere. Empty if neither can be determined.
