...
std::string ProfileStore::default_root() {
    ...
    if (getenv_s(&length, buffer, sizeof(buffer), "APPDATA") == 0 && length > 1) {
        return std::string(buffer) + "\\TeamSpeakReShadeOverlay";
    }
    return {};
#else
    if (const char* xdg = std::getenv("XDG_CONFIG_HOME")) {
        if (*xdg != '\0') return std::string(xdg) + "/tsro";
    }
    if (const char* home = std::getenv("HOME")) {
        if (*home != '\0') return std::string(home) + "/.config/tsro";
    }
    return {};
#endif
}
...
Config ProfileStore::load(std::string_view name, ConfigDiagnostics& diag) const {
    ...
    if (diag.from_defaults) {
        TSRO_WARN(kComponent, "configuration at " + path + " could not be read; using defaults");
    }
    return config;
}
...
bool ProfileStore::save(std::string_view name, const Config& config, std::string& error) const {
    ...
    TSRO_INFO(kComponent, "saved profile '" + std::string(name) + "'");
    return true;
}
